...
const getRecommendedUsers = async (req, res, next) => {
    ...
    try {
        ...
        const following_objs = await User.findById(uid, "following").populate(
            {path:"following", select: "following"})
        ...
        const top_20_similar = Object.keys(filtered_scores).sort((x,y) => filtered_scores[y] - filtered_scores[x]).slice(0,20)

        // Get most popular users
        const user_followers = await User.aggregate([{$project: {followers: {$size: "$followers"}}},{$sort: {followers:-1}}])
        ...
        const error = new HttpError(
            "Could not generate recommended users.",
            404
        );
        return next(error);
    }
...
const getRecommendedProducts = async (req, res, next) => {
    ...
    try {
        ...
        const top_20_similar = Object.keys(item_scores).sort((x,y) => item_scores[y] - item_scores[x]).slice(0,20)
        res.json(top_20_similar)
